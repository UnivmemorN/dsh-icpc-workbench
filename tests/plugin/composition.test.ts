...
test('host composition keeps activation free, persists settings/accounts and creates a restorable backup',async()=>{
 ...
 try {
  assert.equal(f.calls(),0);assert.equal(f.routes.size,63); // 46 business/model/bootstrap + 8 typed Luogu + 3 virtual-performance + 6 assessment operations.
