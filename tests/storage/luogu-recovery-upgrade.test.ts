...
void test('genuine v8 Luogu backlog migrates to the current schema with a verified v8 backup and byte-identical legacy rows',async()=>{
 ...
 try{const store=new SqliteTrainingStore({path:paths.path});try{assert.equal(store.capabilities().schemaVersion,STORE_SCHEMA_VERSION);const row=await store.getLuoguSyncState(account.id);assert.deepEqual(row?.value,state);assert.equal(Object.hasOwn(row!.value,'metadataIssues'),false);assert.equal(row?.revision,1);}finally{await store.close();}
 const db=new DatabaseSync(paths.path,{readOnly:true});let names:readonly string[];try{assert.equal(readUserVersion(db),STORE_SCHEMA_VERSION);assert.equal(db.prepare('SELECT body FROM luogu_sync_states').get()!['body'],bodies.state);assert.equal(db.prepare('SELECT body FROM source_instances').get()!['body'],bodies.source);assert.equal(db.prepare('SELECT body FROM accounts').get()!['body'],bodies.account);names=tableNames(db);}finally{db.close();}
 const backups=readdirSync(paths.dir).filter(x=>x.includes('.backup-v8-')&&x.endsWith('.sqlite'));assert.equal(backups.length,1);const backup=new DatabaseSync(join(paths.dir,backups[0]!),{readOnly:true});try{assert.equal(readUserVersion(backup),8);assert.deepEqual(tableNames(backup),names!.filter(name=>name!=='problem_dispositions'&&name!=='material_refresh_batches'));assert.ok(names!.includes('problem_dispositions'));assert.ok(names!.includes('material_refresh_batches'));assert.equal(backup.prepare('SELECT body FROM luogu_sync_states').get()!['body'],bodies.state);}finally{backup.close();}
