...
薄弱项提供「平台标签参考（未复核）」与「已复核分析」两种视图。正式分析只使用有效标签，每个标签至少 5 道尝试题才进入排名；覆盖不足时可以先查看平台标签的通过/尝试数、未确认通过数和通过率。原始标签可能不完整，参考统计不会自动变成已采纳标签，也不会进入正式训练计划。覆盖诊断显示实际分子/分母及缺失材料的补充入口，查看和切换统计不会自动调用模型。
