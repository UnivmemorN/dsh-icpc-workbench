...
“薄弱项”默认展示 **能力评估（本地启发式训练难度参考，非官方 rating；样本不足显示“未知”）**；“知识点掌握情况”可按名称/别名、分类与证据状态筛选并翻页，非 Codeforces 平台只展示原生难度分位数并标注“CF 估计不适用”。分别列出平台标签参考、已采用标签，以及复盘中确认的独立完成、使用提示、参考题解次数；同一题重复提交或修改复盘不会重复累加。没有记录表示尚未观察，已有 AC 但未确认实际解法会显示待确认；至少 5 道独立完成记录显示“已有独立证据”，不代表所有难度都已掌握。类别只作去重汇总，不向下推定子知识点。
