...
余额显示由可独立装卸的 `@lemcae/dsh-balance` 提供，必须安装在实际运行工作台的 dsh profile 中。ICPC 页面可通过左下角全局「设置 → DeepSeek 余额」查看详细卡片；安装及当前宿主的兼容补丁见 [余额插件配置](docs/dsh-balance-setup.md)。

洛谷同步的待补列表支持逐题或选择本页批量**跳过补题面 / 移入回收站 / 恢复**。回收站持久阻止再次同步显示，并从本地统计、新能力评估和训练选题中排除；原始记录保留。详见[跳过与回收站](docs/problem-recycle-bin.md)。
