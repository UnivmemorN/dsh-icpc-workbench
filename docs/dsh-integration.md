...
Actual host acceptance correction: at 0.1.5-rc.2, the HTTP bridge attaches a body for every streaming route; WHATWG GET forbids a body. Register bodyless GET routes as buffered and POST import/model routes as streaming. This was verified against the real authenticated Web carrier. During local development pnpm may retain an installed file archive when its path/version are unchanged; use a fresh archive filename and inspect installed output before restarting the isolated profile.
