...
- 准备好插件的本地安装包，例如 `dsh-icpc-workbench-0.1.21.tgz`。本文按已有本地 tgz 说明安装；需要自行生成安装包时，请按开发文档从源码构建。请确认文件名与版本一致，并在升级时使用新的文件名（安装器可能缓存同一路径与版本的文件）。
- 系统与插件版本：
...
dsh plugin --profile icpc-training add .\dsh-icpc-workbench-0.1.21.tgz
```
