...
- 准备好插件的本地安装包，例如 `dsh-icpc-workbench-0.1.26.tgz`。本文按已有本地 tgz 说明安装；需要自行生成安装包时，请按开发文档从源码构建。请确认文件名与版本一致，并在升级时使用新的文件名（安装器可能缓存同一路径与版本的文件）。
- 系统与插件版本：
  - Node 声明为 `^22.19.0 || >=24.0.0`；运行环境需满足该版本要求。
  - **Windows**：洛谷本机会话连接可用，凭据保存在 Windows 凭据管理器。
  - **Linux 等没有受支持凭据后端的系统**：账号、题库、JSON/CSV 导入与 AI 功能仍可用，但「连接洛谷 / 验证已保存的 Cookie / 断开连接」会返回固定的不可用提示。
...
dsh plugin --profile icpc-training add .\dsh-icpc-workbench-0.1.26.tgz
```
