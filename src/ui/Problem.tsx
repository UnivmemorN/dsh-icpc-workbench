...
export function ProblemView({problemKey,onChange}:{problemKey:string;onChange:()=>void}){
 ...
 <details><summary>查看当前分析与证据</summary>{problem.analyses?.filter(a=>a.current).map(a=><div key={a.analysisId}><p className="icpc-muted">{a.status} · 词表 {a.taxonomyVersion}</p>{a.suggestions.map(s=><div className="icpc-card" key={s.suggestionId}><strong>{tagName(s.taxonomyId,boot)}</strong><p>{s.rationale}</p>{s.evidence.map((e,i)=><blockquote key={i}>{e.excerpt}<small> · 解法 {e.solutionId}</small></blockquote>)}<p className="icpc-muted">{s.verification?'复核：'+s.verification.verdict:'尚无复核记录'}</p><div className="icpc-actions">{(['accept','reject'] as const).map(choice=><button key={choice} disabled={action.busy} onClick={()=>{void action.run(async signal=>{await api.request('review.tag',{problemKey,taxonomyId:s.taxonomyId,action:choice,note:note||null},signal);changed();return true;});}}>{choice==='accept'?'人工接受':'人工拒绝'}</button>)}</div></div>)}{a.reasoningDrafts.length>0&&<Notice>无题解推理仅供人工审核：{a.reasoningDrafts.map(d=>d.rationale).join('\n')}</Notice>}</div>)}</details>
 ...
 <details><summary>查看保存的题解（{problem.snapshot?.solutionCount??0}）</summary>{problem.snapshot?.solutions?.map(s=><article key={s.solutionId}><h3>{s.title}</h3><pre>{s.text}</pre></article>)}{problem.snapshot?.sources?.map(s=><p key={s.sourceId}><ExternalLink href={s.url}>{s.title}</ExternalLink> · {s.availability}</p>)}</details>
