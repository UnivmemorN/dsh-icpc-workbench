...
export function Review(){
 ...
 const [prepared,setPrepared]=useState<ApiResponse<'batch.prepare'>|null>(null),[batchId,setBatchId]=useState<string|null>(null),[maxJobs,setMaxJobs]=useState(20),[recovered,setRecovered]=useState<ApiResponse<'batch.recover'>|null>(null);
 ...
 <Panel title="标签分析批次"><ErrorNotice error={action.error}/><p>已选 {selectedKeys.length} 题。准备操作只建立任务；开始运行后才调用模型。</p><div className="icpc-toolbar"><label>本批最多题数<input type="number" min="1" max="100" value={maxJobs} onChange={e=>setMaxJobs(Number(e.target.value))}/></label><button disabled={action.busy||!selectedKeys.length||!Number.isInteger(maxJobs)||maxJobs<1||maxJobs>100} onClick={()=>{void action.run(async signal=>{const value=await api.request('batch.prepare',{problemKeys:selectedKeys,maxJobs},signal);setPrepared(value);setBatchId(value.batchId);list.refresh();return value;});}}>免费准备批次</button><button onClick={()=>navigate('bank')}>去题库选择</button></div>
 {prepared&&<Notice><p>{prepared.provider} · 分析 {prepared.models.analysis} · 复核 {prepared.models.verification} · 无题解推理 {prepared.models.reasoning} · max</p><p>已找到题解 {prepared.availability.ready}；已确认无题解 {prepared.availability.absent}；材料不全或获取失败 {prepared.availability.error}。</p><p>本批最多 {prepared.upperBoundCalls.analysisCalls} 次分析/复核、{prepared.upperBoundCalls.reasoningCalls} 次推理（含重试）。这不是费用预测。</p><p>新任务 {prepared.jobs.length}；此前已处理 {prepared.alreadyDone.length}。相同快照的已完成任务不会重复派工。</p></Notice>}
