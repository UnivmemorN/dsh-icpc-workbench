...
export function ImportPanel({sourceId,onChange}:{sourceId:string;onChange:()=>void}){
 ...
 {!canSync&&<Notice>当前来源或账号不支持这里的通用在线同步。{source?.platform==='luogu'?'洛谷提交记录请使用下方「洛谷账号连接与同步」面板（需要先选中一个洛谷账号）。':'尚未实现 HydroOJ 在线适配。'}</Notice>}
 ...
 <LuoguSyncPanel sourcePlatform={source?.platform??null} onChange={onChange}/>
