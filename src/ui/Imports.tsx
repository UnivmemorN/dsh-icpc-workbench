...
import{LuoguSyncPanel}from'./LuoguSync.js';
export function ImportPanel({sourceId,onChange}:{sourceId:string;onChange:()=>void}){
 ...
 const account=boot.accounts.find(a=>a.id===accountId)??null,accountMatchesSource=account!==null&&account.sourceInstanceId===sourceId;
 ...
 /** Submissions describe one account on its own source, so a mismatching account never reaches `sync.page`. */
 const canSync=Boolean(capability?.implemented&&(resource==='problems'?capability.problems:capability.submissions&&accountMatchesSource));
 ...
 {!canSync&&<Notice>{resource==='submissions'&&accountId!==null&&!accountMatchesSource
  ?'所选账号不属于当前来源：提交记录只能同步它自己的来源，请把上方「数据来源」切换到该账号。'
  :'当前来源或账号不支持这里的通用在线同步。'+(source?.platform==='luogu'?'洛谷提交记录请使用下方「洛谷账号连接与同步」面板（需要先选中一个洛谷账号）。':'尚未实现 HydroOJ 在线适配。')}</Notice>}
 ...
 {/* Mounted only for a Luogu source: a Codeforces or manual context has no use for it. */}
 {source?.platform==='luogu'&&<LuoguSyncPanel sourcePlatform={source.platform} onChange={onChange}/>}
