...
export function taskContext(task,root){
  ...
  for(const file of task.contextFiles){
    ...
    if(!/^(?:AGENTS\.md|docs\/(?:architecture|handoffs\/v1|dsh-integration)\.md|\.local\/contract-[\w-]+\.md|(?:src|tests)\/[\w/.-]+\.tsx?)$/.test(file))throw new Error('Context file is outside the source/contract allowlist');
    if(!['.md','.ts','.tsx'].includes(extname(file))||seen.has(file))throw new Error('Invalid or duplicate context file');
